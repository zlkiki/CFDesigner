# [요구사항 08-8] Phase 8: CFS 원본 대화창(frmQuickDesign.cs) 100% 풀스펙 UI 및 3대 D/C 최적설계 엔진 전수 이식 & 교차 검증
...
* **이슈 내용**: 도움말 6-4 및 상용 CFS 원본 대비 현재 웹의 퀵 디자인 모달 파라메터가 축소되어 있어 실무 엔지니어링 조건(단면 6종 필터, 하중 5종, 횡지지, 처짐한계, 지압길이 등)을 온전히 반영하지 못함.
* **목표**:
  1. 원본 `frmQuickDesign.cs`의 모든 입력 항목을 100% 풀스펙 웹 모달로 완벽 이식.
  2. 강도(Strength), 처짐(Deflection), 웨브 크리플링(Web Crippling) 3대 통합 D/C 검토 엔진을 구축하여 최적 경량 단면 실시간 자동 추천.
  3. 원본 C# 계산치와 대조하여 오차 0.1% 미만의 무결성 입증.
...
1. **퀵 디자인 웹 모달 3열 풀스펙 재설계 (`index.html`)**:
   - 좌측: **[단면 및 재료 필터]** (Depth, Type, Flange, Thickness, Punched, Config, Fy, ColdWork, Reserve)
   - 중앙: **[경간 및 설계 하중]** (Span, Spacing, Bracing, Dead, Live, Wind, Axial Dead/Live, Deflection Limit, Bearing Length)
   - 우측: **[최적 단면 랭킹 및 3대 D/C 게이지]** (Strength, Deflection, Web Crippling)
2. **3대 다축 검토 엔진 확장 (`quick_design.py`)**:
   - 강도 검토: 축압축 + 이축휨 P-M 조합 D/C ratio
   - 사용성 처짐: 활하중 $\delta_L \le L/360$, 총하중 $\delta_{total} \le L/240$
   - 웨브 크리플링: 지압길이 $N$ 및 지점 반력 $R$에 대한 $\phi P_n$ 및 D/C ratio
   - 단위중량($\text{kg/m}$) 최소 단면 1위~10위 자동 랭킹 추천.
3. **원본 C# 동일성 교차 검증**:
   - 원본 `frmQuickDesign.cs`의 계산 결과와 0.1% 오차 이내 일치 입증.

---

## 4. 1:1 수용 기준 (Acceptance Criteria)

- [ ] **AC 8-1**: 퀵 디자인 모달 열기 시 CFS 원본 `frmQuickDesign.cs`의 모든 입력 항목(단면 6종 필터, 하중 5종, 지지/처짐/지압길이)이 충실히 제공되는가?
- [ ] **AC 8-2**: 소요 설계조건 입력 후 [⚡ 최적 단면 자동 추천] 클릭 시 강도, 처짐, 웨브 크리플링 3대 D/C를 모두 만족하는 최적 경량 단면 목록이 정상 출력되는가?
- [ ] **AC 8-3**: 추천된 단면 중 하나를 클릭했을 때 메인 캔버스와 설계 화면으로 해당 단면의 제원과 물성치가 원클릭 로드되는가?
- [ ] **AC 8-4**: CFS 원본 계산치와 대조한 단위 테스트 `pytest tests/ui/test_quick_design.py`가 100% Pass 통과하는가?
