...
## 4. Acceptance Criteria (검증 기준 및 1:1 체크리스트)

- [ ] **AC 5-1**: `src/web/static/images/manual/` 디렉토리에 원본 13종 이미지 파일(`section.png`, `analysis.png`, `buckle-*.png`, `torsion-*.png`, `quick-design.png`, `folder-*.jpg`)이 정상 배치되고 웹 브라우저에서 200 OK로 렌더링될 것.
- [ ] **AC 5-2**: `manual.html` 및 `manual.css`에서 도해 이미지가 깨끗하게 렌더링되며, 클릭 시 라이트박스(Lightbox) 확대 모달이 정상 동작할 것.
- [ ] **AC 5-3**: `ui_layout`, `element_grid`, `quick_design` 토픽에 원본 레거시 UI와 신규 AltDP 웹 UI 비교 도해 및 조작법 설명이 포함될 것.
- [ ] **AC 5-4**: `wizard` 및 `analysis_wizard` 토픽에 원본 마법사 2단계 및 4단계 튜토리얼(Step-by-Step Walkthrough)이 한·영 병기로 수록될 것.
- [ ] **AC 5-5**: `dxf_import` 토픽에 AutoCAD DXF 폴리라인 중심선 작도, 호(Arc) 분할 오차 제어 및 두께 부여 실무 가이드가 포함될 것.
- [ ] **AC 5-6**: `buckling_modes`, `signature_curve`, `torsion_props` 토픽에 좌굴 모드 4대 도해 및 비틀림 좌표계 4대 도해가 수록될 것.
- [ ] **AC 5-7**: 원본 `glossary.htm`(18,663자) 및 `symbols.htm`(6,673자) 내용이 누락 없이 온라인 도움말 데이터셋 및 검색 인덱스에 통합될 것.
- [ ] **AC 5-8**: `pytest tests/test_manual_api.py` 및 전체 단위 테스트 suite 55건 이상이 에러 없이 100% 통과할 것.
