...
- [x] **AC 5-2-1**: `wizard` 토픽에 단면 마법사 1~2단계 튜토리얼(C/Z형강 립 및 코너 R 설정법)이 한·영 병기로 수록될 것.
- [x] **AC 5-2-2**: `analysis_wizard` 토픽에 1D 해석 마법사 1~4단계(경간 $\rightarrow$ 지점 $\rightarrow$ 하중 $\rightarrow$ 조합) 실무 예제가 한·영 병기로 수록될 것.
- [x] **AC 5-2-3**: `dxf_import` 토픽에 폴리라인 중심선 및 호 분할 오차 제어 실무 가이드가 보강될 것.
- [x] **AC 5-2-4**: `pytest tests/test_manual_api.py`가 100% 통과할 것.
