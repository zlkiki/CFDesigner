...
- [x] **AC 5-3-1**: `buckling_modes` 및 `signature_curve` 토픽에 원본 좌굴 4종 도해와 변형 모드 판별법이 수록될 것.
- [x] **AC 5-3-2**: `torsion_props` 토픽에 원본 비틀림 4종 도해와 부호규약/바이모멘트 해설이 수록될 것.
- [x] **AC 5-3-3**: `glossary` 및 `symbols` 토픽에 원본 `glossary.htm`(1.8만자) 및 `symbols.htm`(6.6천자) 전수가 한·영 병기로 수록되고 검색될 것.
- [x] **AC 5-3-4**: `pytest tests/test_manual_api.py` 및 전체 단위 테스트 suite가 100% 통과할 것.
