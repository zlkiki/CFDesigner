...
- [ ] **AC 1-1**: 우측 상단 테마 토글 버튼 클릭 시 3D 뷰어 배경색이 라이트 모드(`0xf8fafc`)와 다크 모드(`0x0f172a`)로 즉시 동적 전환되는가?
- [ ] **AC 1-2**: 라이트 모드에서 2D 캔버스 X-Y 원점 원 내부가 흰색 계열로 정상 표시되는가?
- [ ] **AC 1-3**: 라이트 모드에서 2D/3D 캔버스 상단 툴바(확대, 축소, 리셋, 그리드) 박스가 검은색이 아닌 테마 CSS 변수에 맞게 밝은 카드로 표시되는가?
