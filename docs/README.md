...
| **[`15_cfs_legacy_ui_menu_and_dialog_specification.md`](file:///f:/PyProject/CFDesigner/docs/15_cfs_legacy_ui_menu_and_dialog_specification.md)** | CFS 원본 UI/메뉴/창 구성 및 입출력 상세 명세서 | 상용 CFS 원본 7대 메뉴, 3대 MDI 창, 2대 플로팅 입력창, 15대 전문 다이얼로그 및 14종 리포트 입출력 전수 명세 |
| **[`16_frontend_integrity_and_update_prevention_guide.md`](file:///f:/PyProject/CFDesigner/docs/16_frontend_integrity_and_update_prevention_guide.md)** | 프론트엔드 무결성 유지 및 대규모 업데이트 재발 방지 가이드 | 버튼 불통/구문오류 원천 방어, 4대 취약 패턴, 자동화 괄호/구문 검사 파이프라인 및 안전 작업 7대 수칙 |
