...
## 2. 8대 카테고리 및 27개 전수 토픽 인덱스 명세

| 카테고리 | 토픽 ID | 한글 제목 (Web UI) | 영문 제목 (CFS 14.0 Reference) | 주요 수록 내용 및 공학 이론 | 구현 Phase |
|---|---|---|---|---|:---:|
| **1. 시작하기 & 일반 정보** | `intro` | 시스템 소개 및 특징 | System Overview & Features | CFS SaaS 웹 구조, 클라우드 수치해석 아키텍처 개요 | 기반 |
| | `ui_layout` | 웹 UI 4분할 레이아웃 가이드 | Web UI 4-Quadrant Layout & Windows Guide | 4대 작업 영역(제어패널, 2D/3D 캔버스, FSM 차트, D/C 패널) 및 레거시 Section/Analysis 창 | 기반/Ph6 |
| **2. 단면 모델링 & 마법사** | `wizard` | 단면 마법사 파라메트릭 생성 | Parametric Section Wizard | 6대 기본 단면(C, Z, Hat, Deck, Tube, Angle) 파라메트릭 빌드 & 2단계 튜토리얼 | 기반/Ph6 |
| | `dxf_import` | AutoCAD DXF 가져오기 & 메싱 | AutoCAD DXF Import & Meshing Guide | 2D Polyline 중심선 작도, 호 분할, 5대 필수 사양 가이드 | 기반/Ph6 |
| | `element_grid` | 단면 요소 테이블 직접 편집 | Element Table Spreadsheet Editor | 노드, 길이, 각도, 두께 스프레드시트 편집 모달 | Phase 1/Ph6 |
| | `geom_transform` | 단면 기하 변환 및 중간 리브 | Geometric Transforms & Intermediate Stiffeners | 90° 회전, 대칭 미러링, 원점 정렬, V/U형 보강 리브 추가 | Phase 1/Ph6 |
| **3. 단면 라이브러리 & 재료** | `section_lib` | 표준 단면 라이브러리 브라우저 | Section Library Browser (AISI/SSMA/SFIA) | 1,000+개 표준 단면(SSMA/SFIA/LGSI) 검색, 필터 및 로드 | Phase 2/Ph6 |
| | `material_db` | 강재 재료 DB 및 물성치 설정 | Material Properties & Custom Steel Database | KS(SSC275, SGC 등) 및 ASTM 프리셋, $F_y, F_u, E, \nu$ 파라미터 | Phase 2/Ph6 |
| | `cold_work` | 코너 성형 가공경화 강도 증가 | Cold-Work Forming Strength Increase | AISI S100 Appendix 1 코너 가공경화 유효항복강도($F_{ya}$) 산정식 | Phase 2/Ph6 |
| **4. 단면 성질 & 유효단면** | `gross_props` | 총단면 기하학적 성질 (Gross) | Gross Section Properties (Gross Characteristics) | $A_g, I_x, I_y, r_x, r_y$, 도심($C_G$) 선적분 메싱 적분 공식 | 기반/Ph6 |
| | `torsion_props` | 비틀림 및 뒴 성질 (Torsion) | Torsional & Warping Properties (Vlasov Theory) | 생브낭 $J$, 뒴상수 $C_w$, 전단중심 $S_C(x_0, y_0)$, 극반경 $r_0$ & 4대 도해 | 기반/Ph6 |
| | `principal_axes` | 주축 및 주단면 2차모멘트 | Principal Axes & Principal Second Moments | 주축 회전각($\theta_p$), 주단면 2차모멘트($I_1, I_2$), 단면 계수 | 기반/Ph6 |
| | `effective_props` | Winter 식 기반 유효단면 해석 | Effective Section Properties (Winter Method) | Winter 판폭 감소식($\rho$), 중립축 반복 계산 및 2D 점선 표시 | Phase 3/Ph6 |
| **5. FSM 탄성 좌굴해석** | `fsm_theory` | FSM 탄성 좌굴 해석 이론 | Finite Strip Method (FSM) Buckling Theory | $[K_e], [K_g]$ 대판 강성행렬 유도 및 일반화 고유치 수치해석 | 기반/Ph6 |
| | `buckling_modes` | 좌굴 모드 판별 (국부/왜곡/전체) | Buckling Mode Classification (Local/Distortional/Global) | $P_{crl}, P_{crd}, P_{cre}$ 판독법 및 4대 좌굴 도해 | 기반/Ph6 |
| | `signature_curve` | 시그니처 커브 및 3D 시각화 | Signature Curve & 3D Buckling Visualization | 반파장 $L$ 스펙트럼 곡선, Three.js 3D 모드 형상 조작 | 기반/Ph6 |
| | `fsm_params` | FSM 해석 구간 및 하중조건 설정 | Buckling Analysis Parameters & Results Grid | 스윕 범위($L_{min} \sim L_{max}$), 응력 구배 설정, CSV 내보내기 | Phase 3/Ph6 |
| **6. KDS / AISI 부재설계** | `kds_dsm_comp` | KDS 압축부재 설계 (DSM Pn) | Compression Member Design (Direct Strength Method) | KDS / AISI DSM 압축강도 $P_{ne}, P_{nl}, P_{nd}, P_n, \Omega_c, \phi_c$ 산정식 | 기반/Ph6 |
| | `kds_dsm_flex` | KDS 휨부재 설계 (DSM Mn) | Flexural Member Design (Direct Strength Method) | KDS / AISI DSM 휨강도 $M_{ne}, M_{nl}, M_{nd}, M_n, \Omega_b, \phi_b$ 산정식 | 기반/Ph6 |
| | `kds_shear_crip` | KDS 전단강도 & 웨브 크리플링 | Shear & Web Crippling Strength (AISI S100 Provisions) | 전단강도 $V_n$ 및 4대 지지조건(EOF/IOF/ETF/ITF) $P_{nc}$ 계수표/산정식 | Phase 3/Ph6 |
| | `quick_design` | 퀵 디자인 (최적 단면 자동 추천) | Quick Design Optimization Tool | 소요 하중($P_u, M_u$) 입력 시 경량 최적 단면 자동 탐색 & 레거시 도해 | Phase 3/Ph6 |
| | `kds_interaction` | P-M 조합응력 & D/C 검토 | P-M Interaction & Combined Stress Verification | 휨-압축 상관식, 모멘트 증대계수($B_1$), D/C 판정 기준 | 기반/Ph6 |
| | `report_guide` | A4 구조계산서 출력 & 인쇄 | A4 Calculation Report & Export Guide | 인쇄 미리보기, 수식 근거표, 브라우저 PDF/인쇄 표준 서식 | 기반/Ph6 |
| **7. 1D 구조해석** | `analysis_wizard` | 1D 구조해석 마법사 & 하중입력 | 1D Frame Analysis Wizard & Loadings Guide | 4단계(경간/지점/하중/조합) 모델링 및 1D 해석 튜토리얼 | Phase 4/Ph6 |
| | `diagrams_viewer` | SFD / BMD / 처짐 다이어그램 | Shear, Moment & Deflection Diagrams (SFD / BMD) | SFD/BMD/처짐 차트 인터랙션 및 부재설계 원클릭 연동 흐름 | Phase 4/Ph6 |
| **8. 부록: 사전 & 기호집** | `glossary` | 냉간성형강 전문 용어사전 | Engineering Glossary (AISI S100 & KDS) | AISI S100/KDS 1.8만자 A~Z 공학 전문 용어 공식 정의집 | Phase 5/Ph6 |
| | `symbols` | 공학 기호 및 약어 정의집 | Symbols & Abbreviations | 로마자/그리스 문자/설계 약어 114종 기호 및 단위계 총람 | Phase 5/Ph6 |
