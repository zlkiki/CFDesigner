...
개발 작업 시 전체 60+개 테스트를 매번 실행하는 오버헤드를 줄이고, **작업 중인 도메인에만 집중하여 0.5~2초 내에 즉각적인 피드백**을 얻을 수 있도록 테스트 스위트를 3대 영역으로 분리 운영합니다.

```text
tests/
├── engine/          # ⚙️ [도메인 1] 순수 공학/수치해석/설계식 엔진 검증 (13 tests)
...
│
├── ui/              # 💻 [도메인 2] 웹 UI 연동, API 엔드포인트 및 JSON 스키마 (13 tests)
...
│
└── manual/          # 📖 [도메인 3] 온라인 도움말 시스템 및 정적 자산 무결성 (35 tests)
    └── test_manual_api.py               # 8대 카테고리 트리, 27개 토픽 한/영 원문 대칭성, 도해 링크, 수식 무결성
```
...
|---|---|---|:---:|
| **엔진 / 공학 수식 작업** | `pytest tests/engine/` | 단면 특성치($A, I, J, C_w$), FSM 좌굴하중계수, KDS 공칭강도 | **~1.5초** |
| **웹 UI / API 라우트 작업** | `pytest tests/ui/` | FastAPI 엔드포인트, 클라이언트 요청/응답 스키마, 기하 편집 | **~0.7초** |
| **온라인 도움말 / 문서 작업** | `pytest tests/manual/` | 한/영 8대 트리 구조, 도해 이미지 경로, 수식($$) 렌더링 무결성 | **~1.9초** |
| **마스터 요구사항 완료 직전** | `pytest` | 전체 3대 영역 일괄 회귀 방지 통합 검증 (61 tests) | **~19초** |
...
2. **웹 UI / API 테스트 (`tests/ui/`)**:
   ...
   - 단면 편집, 회전, 대칭, SSMA 라이브러리 선택 등 사용자의 UI 액션에 대응하는 API 흐름을 검증합니다.

3. **온라인 도움말 테스트 (`tests/manual/`)**:
   - 8대 대분류 트리와 27개 개별 토픽의 한/영 대조(`content_ko`, `content_en`) 누락 여부를 검증합니다.
   - 본문에 포함된 도해 이미지(`/static/manual_images/...`)의 파일 시스템 실재 여부 및 LaTeX 수식(`$$...$$`)의 유효성을 검증합니다.
